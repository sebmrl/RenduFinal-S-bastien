...
if(isset($_GET['id']) AND !empty($_GET['id']))
{
    ...
    $tweet = $bdd->prepare('SELECT * FROM post WHERE post_id= ?');
    $tweet->execute(array($get_id));

    if($tweet->rowCount() > 1)
    {
        $tweet = $tweet->fetch();
        $titre = $tweet['post_titre'];
        $contenu = $tweet['post_contenu'];
    }
    else
    {
        die('Ce tweet n\'existe pas !');
    }
}
                ...
                <ul class="list_post">
                    ...
                    <?php while($a = $post->fetch()) { ?>
                        <li class="post">
                            ...
                            <a href="supprimer.php?id=<?=$a['post_id'] ?>"> <input type="button" name="suppr" value="Supprimer"> </a>
                        </li>
                        <?php } ?>
